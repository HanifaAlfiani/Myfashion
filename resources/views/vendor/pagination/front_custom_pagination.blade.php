...
<nav aria-label="Page navigation example">
    <ul class="pagination pagination-circle">
        ...
        @else
            <li class="page-item"><a class="page-link" href="{{ $paginator->previousPageUrl() }}" style="color: #232d68;" rel="prev">Previous</a></li>
        @endif
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li class="page-item active"><a href="" class="page-link" style="background-color: #ccccce; border: 1px solid #acacac;">{{ $page }}</a></li>
                    @else
